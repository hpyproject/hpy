                                              ...
                                              void *func, void *args)
{
    ...
    case HPyFunc_NOARGS: {
        ...
        DHPy dh_self = HPy_FromPyObject(dctx, a->self);
        DHPy dh_result = f(dctx, dh_self);
        a->result = _dh2py(dh_result);
        HPy_Close(dctx, dh_self);
    ...
    case HPyFunc_O: {
        ...
        DHPy dh_self = HPy_FromPyObject(dctx, a->self);
        DHPy dh_arg = HPy_FromPyObject(dctx, a->arg);
        DHPy dh_result = f(dctx, dh_self, dh_arg);
        a->result = _dh2py(dh_result);
        HPy_Close(dctx, dh_self);
        HPy_Close(dctx, dh_arg);
        DHPy_close(dctx, dh_result);
        return;
    }
    case HPyFunc_VARARGS: {
        ...
        for (Py_ssize_t i = 0; i < nargs; i++) {
            dh_args[i] = HPy_FromPyObject(dctx, PyTuple_GET_ITEM(a->args, i));
        }
        DHPy dh_result = f(dctx, dh_self, dh_args, nargs);
        a->result = _dh2py(dh_result);
        HPy_Close(dctx, dh_self);
        for (Py_ssize_t i = 0; i < nargs; i++) {
            HPy_Close(dctx, dh_args[i]);
        }
        DHPy_close(dctx, dh_result);
        return;
    }
    case HPyFunc_KEYWORDS: {
        ...
        DHPy dh_kw = HPy_NULL;
        if (a->kw != NULL) {
            dh_kw = HPy_FromPyObject(dctx, a->kw);
        }
